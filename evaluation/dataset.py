...
from utils.uea import subsample, interpolate_missing, Normalizer, collate_fn
    ...
    if score_key == "random":
        ...
        with open(score_file, 'r') as f:
            for line in f:
                ...
                index_to_score[rec["index"]] = rec["quality_score"]

        scores = np.array([index_to_score.get(i, 0.0) for i in range(total_samples)])
        selected_indices = np.argsort(-scores)[:num_samples].tolist()
